...
// Lazy load non-critical views to reduce initial bundle size
const PrivacyContent = lazy(() => import('./components/PrivacyContent').then(m => ({ default: m.PrivacyContent })));
const TermsContent = lazy(() => import('./components/TermsContent').then(m => ({ default: m.TermsContent })));
const AboutContent = lazy(() => import('./components/AboutContent').then(m => ({ default: m.AboutContent })));
const DiagnosticsContent = lazy(() => import('./components/DiagnosticsContent').then(m => ({ default: m.DiagnosticsContent })));
...
const ViewLoader = () => (
  <div className="flex-1 flex items-center justify-center">
    <div className="bg-white dark:bg-zinc-900 neubrutal-border p-8 font-black uppercase tracking-widest animate-pulse">
      Loading Module...
    </div>
  </div>
);
...
export default function App() {
  ...
  const processConversion = useCallback(async (id: string) => {
    ...
    try {
      ...
      // OPTIMIZATION: Use dynamic imports for utility functions to reduce initial JS payload
