...
const isImage = (f: File) => f.type.startsWith('image/') && !f.type.includes('svg') && !f.name.toLowerCase().endsWith('.heic');
const isHeic = (f: File) => f.name.toLowerCase().endsWith('.heic') || f.name.toLowerCase().endsWith('.heif');
const isSvg = (f: File) => f.type.includes('svg') || f.name.toLowerCase().endsWith('.svg');
...
const isYaml = (f: File) => f.name.toLowerCase().endsWith('.yaml') || f.name.toLowerCase().endsWith('.yml');
const isXml = (f: File) => f.type.includes('xml') || f.name.toLowerCase().endsWith('.xml');
const isXlsx = (f: File) => f.name.toLowerCase().endsWith('.xlsx') || f.name.toLowerCase().endsWith('.xls');
const isMd = (f: File) => f.name.toLowerCase().endsWith('.md') || f.name.toLowerCase().endsWith('.markdown');
const isHtml = (f: File) => f.type.includes('html') || f.name.toLowerCase().endsWith('.html');
const isCss = (f: File) => f.type.includes('css') || f.name.toLowerCase().endsWith('.css');
const isText = (f: File) => f.type.startsWith('text/') || f.name.toLowerCase().endsWith('.txt') || isJson(f) || isCsv(f) || isMd(f) || isHtml(f) || isCss(f);
const isAudio = (f: File) => f.type.startsWith('audio/') || /\.(mp3|wav|ogg|m4a|aac|flac)$/i.test(f.name);
const isVideo = (f: File) => f.type.startsWith('video/') || /\.(mp4|webm|mov|avi|mkv)$/i.test(f.name);
